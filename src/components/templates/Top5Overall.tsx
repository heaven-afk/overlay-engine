...
function resolvePoints(team: any, col: string): { value: string; label: string } {
  switch (col) {
    case 'kills':
      return { value: String(team.kills ?? team.scores?.kills ?? 0), label: 'TOTAL KILLS' };
    case 'placementPts':
      const plPts = team.placementPoints ?? team.placementPts ?? team.placePts ?? team.scores?.placementPts ?? 0;
      return { value: String(plPts), label: 'PLACE PTS' };
    case 'totalPts':
    default:
      const totPts = team.totalPoints ?? team.totalPts ?? team.scores?.totalPts ?? 0;
      return { value: String(totPts), label: 'TOTAL PTS' };
  }
}
...
export const Top5Overall: React.FC<Top5OverallProps> = ({ data, styleConfig }) => {
  const { graphicTitle, graphicSubtitle, topN = 5, showStatsStamp, dailyPointsColumn = 'totalPts' } = styleConfig;
